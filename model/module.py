...
class LFMfilterLayer(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.complex_weight1 = nn.Parameter(torch.randn(1, args.hidden_size, args.MAX_ITEM_LIST_LENGTH//2 + 1, 2, dtype=torch.float32) * 0.02)
        self.out_dropout = nn.Dropout(args.freq_dropout_prob)
        self.conv_layers = args.conv_layers
        self.hidden_size = args.hidden_size
        self.kernel_size = args.kernel_size
        self.LayerNorm = nn.LayerNorm(args.hidden_size, eps=1e-12)
        self.num_bands = args.num_bands
        self.LFMgate = LFMGate(args)
