...
class RecDataset(Dataset):
    def __init__(self, args, user_seq,  test_neg_items=None, data_type='train', topk_trust_dict=None):
        self.args = args
        self.user_seq = []
        # self.short_seq = []
        ...
        # self.topk_trust_dict = topk_trust_dict
    ...
    def __getitem__(self, index):
        ...
        if self.data_type in ['valid', 'test']:
            ...
            cur_tensors = (
                # user_id, seq, answer, neg_answer, sem_aug
...
def get_seq_dic(args):
    ...
    # user_seq, short_seq, max_item, num_users = get_user_seqs(args.data_file)
...
def get_dataloder(args, seq_dic, topk_trust_dict=None):
    train_seq = seq_dic['user_seq']
    # train_short = seq_dic['short_seq']
    
    # 为所有数据集（训练、验证、测试）传递 trust_matrix 和 topk_trust_dict
    train_dataset = RecDataset(
        # args, train_seq, train_short, data_type='train', 
        args, train_seq, data_type='train', 
        # topk_trust_dict=topk_trust_dict
    )
    eval_dataset = RecDataset(
        args, train_seq, data_type='valid', 
        # topk_trust_dict=topk_trust_dict
    )
    test_dataset = RecDataset(
        args, train_seq, data_type='test', 
        # topk_trust_dict=topk_trust_dict
    )
