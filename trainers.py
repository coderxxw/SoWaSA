...
from metrics import recall_at_k, ndcg_k, mrr_at_k
...
class Trainer:
    def __init__(self, model, train_dataloader, eval_dataloader, test_dataloader, args, logger):
        ...
        self.model = model
        # print(112312,self.args.model_type)
    ...
    def get_full_sort_score(self, epoch, answers, pred_list):
        ...
        post_fix = {
            "Epoch": epoch,
            "HR":{'@5': '{:.4f}'.format(recall[0]), '@10': '{:.4f}'.format(recall[1]), '@15': '{:.4f}'.format(recall[2]), '@20': '{:.4f}'.format(recall[3])},
            "NDCG":{'@5': '{:.4f}'.format(ndcg[0]), '@10': '{:.4f}'.format(ndcg[1]), '@15': '{:.4f}'.format(ndcg[2]), '@20': '{:.4f}'.format(ndcg[3])},
            "MRR":{'@5': '{:.4f}'.format(mrr[0]), '@10': '{:.4f}'.format(mrr[1]), '@15': '{:.4f}'.format(mrr[2]), '@20': '{:.4f}'.format(mrr[3])}
        }
    ...
    def iteration(self, epoch, dataloader, train=True):
        ...
        if train:
            ...
            for i, batch in rec_data_iter:
                ...
                if self.args.model_type == "bert4rec":
